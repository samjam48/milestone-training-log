...
def test_update_focus_title_regenerates_active_weekly_focus_name(
    app_with_test_database: FastAPI,
) -> None:
    ...
    seed_weekly_focus_block(
        app_with_test_database,
        block_id="blk-wf-title",
        focus_series_id="fs-title",
        focus_title="Before title",
        week_number=2,
    ...
    for session in with_session(app_with_test_database):
        updated = service.update_focus_title(session, "blk-wf-title", "After title")

    assert updated.focus_title == "After title"
    assert updated.name == "After title · Week 2"
    assert updated.start_date == WEEK_ONE_START
    assert updated.end_date == WEEK_ONE_END


def test_reset_focus_series_delegates_to_weekly_focus_service(
    app_with_test_database: FastAPI,
) -> None:
    from app.tests.helpers.weekly_focus_fixtures import (
        SUNDAY_AS_OF,
    ...
    seed_weekly_focus_block(
        app_with_test_database,
        block_id="blk-wf-reset-delegate",
        focus_series_id="fs-delegate",
        focus_title="Delegate focus",
        week_number=3,
        start_date=WEEK_ONE_START,
        end_date=WEEK_ONE_END,
        status="active",
    ...
    for session in with_session(app_with_test_database):
        ...
        prior = session.get(TrainingBlock, "blk-wf-reset-delegate")

    assert prior is not None
    assert prior.status == "completed"
    assert reset_block.week_number == 1
    assert reset_block.focus_title == "Delegated reset"
