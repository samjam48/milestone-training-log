...
Core rules:
- Keep server data retrieval in API or query modules.
- Avoid duplicating API data in local component state unless there is a clear editing or interaction reason.
- Prefer a single transformation point for shared derived data such as dashboard summaries, weekly targets, or risk states.
