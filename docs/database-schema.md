...
- `id`
- `user_id`
- `name` — calendar week display label (e.g. `Jun 2 – Jun 8, 2026`); generated by the service from Mon–Sun bounds
- `start_date`
- `end_date` nullable
- `status` — active, completed, archived
- `period_kind` — always `weekly_focus` (default `weekly_focus`; legacy values removed by WRU.B1)
- `focus_series_id` nullable — internal grouping for consecutive weeks; used by rollover logic
- `focus_title` nullable — internal column; not user-edited in UI (may be null)
- `week_number` nullable — internal 1-based counter within `focus_series_id`
...
- The main planning container for **weekly rules**, targets, and milestone reviews
- Each row is one Monday–Sunday planning period. Rules and `weekly_targets` are owned per week; Monday rollover copies enabled rules and targets into the next week.
- **Big-bang migration (WRU.B1, Alembic `20260608_0006`):** on upgrade, delete all existing `training_blocks` rows (and dependent rules/targets). Copy enabled rules and `weekly_targets` from the single active legacy block (if any) into one new `weekly_focus` row for the current calendar week. No conversion of old blocks into reviewable history.
...
- **Auto-create (WRU.B2):** `ensure_active_weekly_focus` runs before active-block resolution. When no row exists for the calendar week containing `as_of`, the service creates an empty `weekly_focus` row (week 1, new `focus_series_id`) — no manual setup endpoint.
- **Weekly rules rollover (WRU.B2):** when `as_of` enters a new calendar week, the service completes the prior active `weekly_focus` (`end_date` = its Sunday) and creates the next week with `week_number + 1`, same `focus_series_id`, copying enabled `rules` and all `weekly_targets` (new child ids). Multi-week gaps create one completed period per missed week.
- **Block end_date on archive:** the service sets `end_date` on the outgoing block when completing a period. `end_date` is always that period's Sunday. This ensures `BlockReviewScreen` can scope queries to a closed date range.
- **Weekly progress cadence:** dashboard `weekly_progress` sums activity logs in the Monday–Sunday ISO week containing `as_of`, scoped per `weekly_targets` row (activity-scoped or legacy class-scoped). Weekly rules align planning ownership (which rules/targets apply) with the same Monday–Sunday window. Load risk and load-tax graph use rolling seven-day windows instead — see `docs/api-map.md`.
- **Removed paths (WRU.F2):** `POST /api/training-blocks`, `POST …/active/setup`, `POST …/active/reset-focus`, and user-editable `focus_title` — weekly periods are system-managed only.
