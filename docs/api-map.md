...
| `GET` | `/api/training-blocks/{block_id}/review` | Per-week review payload: `block`, `daily_scores`, `load_series` (combined weighted performance load tax over the block date range — same helper as dashboard), `flare_up_dates`, `total_sessions`, `clean_days` — scoped to that week’s Mon–Sun range; used by `BlockReviewScreen` and `BlockSafetyMapSection`. **Removed (B10.4):** `GET /api/training-blocks/{block_id}/scores` — callers use `daily_scores` from `/review` instead |
...
**Read shape (`ActivityClassRead`):** `id`, `name`, `description`, `type`, `default_recovery_window_days`, `load_weight` (float `0..10`, default `1.0`; scales combined load-tax contribution on dashboard/block-review series; `0` mutes), `created_at`. Also embedded on `GET /api/dashboard` as `activity_classes`.
...
| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/load/summary` | Class statuses, suggestions, weekly progress; optional `?as_of=` (default server-local today); snake_case JSON; resolves active planning period via `ensure_active_weekly_focus` (auto-creates current week when missing) before building derived fields |
...
**`load_series` (load tax graph):** Combined weighted performance load tax. Dashboard: last **30 calendar days** ending on `as_of`. Block review: the block’s date range. Each point: `date`, `daily_load` (Σ `load_weight(C)` × daily load tax for each performance class), `load` (recency-weighted rolling **seven-day** sum of those combined dailies). Recovery classes contribute `0`. `graph_class_id` is always `null`. Raw `volume × rpe` is not used. Weight does **not** apply to `rule_limit_rows`, weekly progress, or suggestion buckets.
...
| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/mcp/context` | Structured AI-readable summary for future MCP integration; optional `?as_of=YYYY-MM-DD` (default server-local today). Resolves active planning period via `ensure_active_weekly_focus` before building fields. Top-level fields: `active_block` (slim weekly rules summary — `id`, calendar-week `name`, Mon–Sun `start_date`/`end_date`, `status`, `period_kind = weekly_focus`, optional internal `focus_series_id` / `focus_title` / `week_number`, `is_review_milestone_hit`), `recent_logs` (last 7 calendar days ending on `as_of`; each entry has `activity_name`, `load_score` as `volume × rpe` with default RPE 5, and `logged_date`), `today_check_in` (`pain`, `readiness`, `stiffness`, `has_flare_up` or `null`), `class_statuses` (slim traffic-light summaries: `activity_class_id`, `state`, `reason`). No auth, no AI calls |
